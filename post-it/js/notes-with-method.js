...
function updateSection(section) {
    ...
    for (let i = 0; i < notes.length; i++) {
        if (!notes[i].editing) {
            contentNotes += '<form id="note-' + i + '" class="note" onclick="updateForm(' + i + ', this.parentElement)">' +
                '<button class="note__control" type="button" onclick="removeNote(event,' + i + ', this.form.parentElement)">' +
                '<i class="fa fa-times" aria-hidden="true"></i>' +
                '</button>' +
            ...
            contentNotes += '<form class="note">' +
                ...
                '<button class="note__control" type="button" onclick="updateNote(' + i + ', this.form.titulo, this.form.texto, this.form.parentElement)">Alterar</button>' +
                '</form>';
        }
    }
